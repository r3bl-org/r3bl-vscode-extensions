...
r3bl-shared is a VSCode extension (NOT an npm package) that provides **centralized
services** with shared state across all R3BL extensions.

**Why?** Each VSCode extension is bundled separately by webpack. If multiple extensions
import the same npm package, each gets its own isolated copy - no shared state possible.
By running as a VSCode extension, r3bl-shared provides true cross-extension communication.
...
- **Smart duration** - automatic timing based on message type (error: 5s, warning: 4s,
  info/success: 3s)
...
**Note**: The error fallback should rarely trigger since all R3BL extensions declare
`extensionDependencies: ["R3BL.r3bl-shared"]`, ensuring r3bl-shared loads first.

### Alternative: Using r3bl-common-code Utilities

For a simplified approach, you can use the **r3bl-common-code** package which provides

...
## Description
Implement a feature in `r3bl-semantic-config` that scrolls the editor viewport so that the current cursor line is at the top of the screen when `Ctrl+M` is pressed.

## Todo List

### Implementation
- [x] Implement `r3bl-semantic-config.scrollToTop` command in `packages/r3bl-semantic-config/src/extension.ts`.
- [x] Register the command in `packages/r3bl-semantic-config/package.json`.
- [x] Add `Ctrl+M` keybinding in `packages/r3bl-semantic-config/package.json` with `when: "editorTextFocus"`.
...
- [x] Update `packages/r3bl-extension-pack/README.md` to document the new `Ctrl+M` shortcut.
